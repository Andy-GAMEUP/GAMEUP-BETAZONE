...
const statusClass: Record<TesterStatus, string> = {
  '승인됨': 'bg-green-500/20 text-green-400 border border-green-500/50',
  '대기중': 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/50',
  '거절됨': 'bg-red-500/20 text-red-400 border border-red-500/50',
}

const STATS = [
  { label: '총 테스터', value: '12,450', color: 'text-white' },
  { label: '승인됨',   value: '10,230', color: 'text-green-400' },
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-lg overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-slate-800">
          ...
          <button onClick={handleExportCSV}
            className="flex items-center gap-2 px-3 py-2 text-sm border border-slate-700 rounded-md hover:bg-slate-800 transition-colors">
            ...
            <thead>
              <tr className="border-b border-slate-800 text-left">
                {['테스터', '게임', '상태', '피드백', '버그 리포트', '평점', '가입일', '작업'].map(h => (
                  <th key={h} className="px-4 py-3 text-sm text-slate-400 font-medium">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map(tester => (
                <tr key={tester.id} className="border-b border-slate-800 hover:bg-slate-800/30 transition-colors">
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-green-600 flex items-center justify-center text-sm font-bold flex-shrink-0">
                        {tester.name[0]}
                      </div>
                      <div>
                  ...
                  <td className="px-4 py-4">
                    <span className="text-xs px-2 py-1 rounded-full border border-green-500/50 text-green-400">{tester.game}</span>
                  </td>
                  <td className="px-4 py-4">
                  ...
                  <td className="px-4 py-4 text-slate-400 text-sm">{tester.bugs}</td>
                  <td className="px-4 py-4">
                    {tester.rating > 0
                  ...
                  <td className="px-4 py-4 text-slate-400 text-sm">{tester.joinDate}</td>
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-2">
                      {tester.status === '대기중' && (
                        <>
                          <button onClick={() => updateStatus(tester.id, '승인됨')}
                            className="flex items-center gap-1 px-2 py-1.5 text-xs bg-green-600 hover:bg-green-700 rounded-md transition-colors">
                      ...
                      {tester.status === '승인됨' && (
                        <button className="px-2 py-1.5 text-xs border border-slate-700 rounded-md hover:bg-slate-800 transition-colors">
                          상세보기
                        </button>
                      )}
                    </div>
