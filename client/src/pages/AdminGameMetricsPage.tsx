...
const FEEDBACK_COLORS: Record<string, string> = {
  general: 'bg-slate-500', bug: 'bg-red-500', suggestion: 'bg-blue-500', praise: 'bg-green-500'
}
const FEEDBACK_TEXT: Record<string, string> = {
  general: 'text-slate-400', bug: 'text-red-400', suggestion: 'text-blue-400', praise: 'text-green-400'
}
const SEVERITY_COLORS: Record<string, string> = {
  low: 'bg-green-500/70', medium: 'bg-yellow-500/70', high: 'bg-orange-500/70', critical: 'bg-red-600'
}
...
const GAME_STATUS: Record<string, { label: string; cls: string }> = {
  ...
  archived:  { label: '종료됨',   cls: 'bg-slate-700/60 text-slate-500' },
}
      ...
      {data.map((d) => (
        <div key={d.label}>
          <div className="flex items-center justify-between mb-1">
            <span className="text-slate-400 text-xs">{d.label}{d.sub ? <span className="text-slate-600 ml-1">{d.sub}</span> : null}</span>
...
export default function AdminGameMetricsPage() {
  ...
  if (!data) return (
    <AdminLayout>
      <div className="text-center py-16">
        ...
        <button onClick={() => navigate('/admin/games')} className="mt-4 text-cyan-400 hover:text-cyan-300 text-sm">← 게임 목록으로</button>
      </div>
    </AdminLayout>
  )
  ...
  const approvalBadge = APPROVAL_STATUS[game.approvalStatus] || { label: game.approvalStatus, cls: 'bg-slate-700 text-slate-400' }
  const statusBadge = GAME_STATUS[game.status] || { label: game.status, cls: 'bg-slate-700 text-slate-400' }
  ...
  const kpiCards = [
    ...
    { icon: Shield,  color: 'text-green-400',  bg: 'bg-green-500/10',  label: '인증 테스터',     value: verifiedTesterCount.toLocaleString() },
    { icon: Heart,   color: 'text-pink-400',   bg: 'bg-pink-500/10',   label: '즐겨찾기',        value: favoriteCount.toLocaleString() },
    { icon: Calendar, color: 'text-slate-300', bg: 'bg-slate-700/40',  label: '총 플레이시간',   value: totalPlayTime > 0 ? `${totalPlayTime.toLocaleString()}분` : '-' },
  ]
  ...
  return (
    <AdminLayout>
      {/* Toast */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 px-4 py-3 rounded-lg shadow-lg text-sm font-medium flex items-center gap-2 ${toast.ok ? 'bg-green-600' : 'bg-red-600'} text-white`}>
      ...
      {confirm && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-sm p-6 shadow-2xl">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-3 ${confirm.danger ? 'bg-red-500/20' : 'bg-orange-500/20'}`}>
              <AlertTriangle className={`w-5 h-5 ${confirm.danger ? 'text-red-400' : 'text-orange-400'}`} />
            </div>
            <h3 className="text-white font-bold text-lg mb-1">{confirm.title}</h3>
            <p className="text-slate-400 text-sm mb-5">{confirm.desc}</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setConfirm(null)} className="px-4 py-2 text-sm text-slate-400 border border-slate-700 rounded-lg hover:bg-slate-800 transition-colors">취소</button>
              <button
                onClick={() => handleControl(confirm.action)}
                className={`px-4 py-2 text-sm text-white rounded-lg transition-colors ${confirm.danger ? 'bg-red-700 hover:bg-red-800' : 'bg-orange-700 hover:bg-orange-800'}`}
              >
                확인
              </button>
            </div>
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-4">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <button onClick={() => navigate('/admin/games')} className="text-slate-400 hover:text-white transition-colors flex-shrink-0">
              <ArrowLeft className="w-5 h-5" />
            </button>
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-white text-xl font-bold truncate">{game.title}</h1>
              ...
              <p className="text-slate-500 text-sm mt-0.5">
                {game.genre} · 개발자: <span className="text-slate-400">{(game.developerId as any)?.username}</span> · 등록 {new Date(game.createdAt).toLocaleDateString('ko-KR')}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {game.status !== 'archived' && (
              <button
                ...
                disabled={actionLoading}
                className="flex items-center gap-1.5 bg-slate-700/50 text-slate-400 border border-slate-600/40 hover:bg-slate-700 px-3 py-1.5 rounded-lg text-xs transition-colors disabled:opacity-50"
              >
                <Archive className="w-3.5 h-3.5" /> 아카이브
              </button>
            )}
            <Link
              to={`/admin/community?gameId=${id}`}
              className="flex items-center gap-1.5 bg-slate-800 text-slate-400 border border-slate-700 hover:border-slate-600 hover:text-white px-3 py-1.5 rounded-lg text-xs transition-colors"
            >
        ...
        <div className="flex gap-1 bg-slate-900 border border-slate-800 rounded-xl p-1 w-fit">
          {([['overview', '개요 분석'], ['reviews', '리뷰 목록'], ['feedback', '피드백 분석']] as const).map(([key, label]) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${tab === key ? 'bg-slate-700 text-white' : 'text-slate-400 hover:text-white'}`}
            >
              {label}
            </button>
          ))}
        ...
        {tab === 'overview' && (
          <div className="space-y-4">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                ...
                <div className="flex gap-1">
                  {(['7d', '30d'] as const).map((m) => (
                    <button key={m} onClick={() => setTrendMode(m)}
                      className={`px-2.5 py-1 rounded text-xs font-medium transition-colors ${trendMode === m ? 'bg-cyan-600/30 text-cyan-300 border border-cyan-500/40' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                      {m === '7d' ? '7일' : '30일'}
                    </button>
                  ))}
              ...
              {trendData.length === 0 ? (
                <p className="text-slate-500 text-sm text-center py-10">플레이 기록 없음</p>
              ) : (
                <>
                  ...
                  {trendMode === '7d' && weeklyTrend.length > 0 && (
                    <div className="mt-4 grid grid-cols-3 gap-3 pt-4 border-t border-slate-800">
                      <div className="text-center">
                        <p className="text-white font-bold">{weeklyTrend.reduce((s: number, d: any) => s + d.plays, 0).toLocaleString()}</p>
                        <p className="text-slate-500 text-xs">7일 총 플레이</p>
                      </div>
                      <div className="text-center">
                        <p className="text-white font-bold">
                          {Math.round(weeklyTrend.reduce((s: number, d: any) => s + d.plays, 0) / weeklyTrend.length).toLocaleString()}
                        </p>
                        <p className="text-slate-500 text-xs">일 평균</p>
                      </div>
                      <div className="text-center">
                        <p className="text-white font-bold">
                          {Math.max(...weeklyTrend.map((d: any) => d.plays)).toLocaleString()}
                        </p>
                        <p className="text-slate-500 text-xs">최고 플레이</p>
                      </div>
                    </div>
                  )}
                </>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                ...
                {totalReviews === 0 ? (
                  <p className="text-slate-500 text-sm text-center py-8">리뷰 없음</p>
                ) : (
                  <>
                    <div className="space-y-2">
                      {[...ratingDist].reverse().map((r: any) => (
                        <div key={r.rating} className="flex items-center gap-2">
                          ...
                          <span className="text-slate-400 text-xs w-5 text-right">{r.count}</span>
                        </div>
                      ))}
                    </div>
                    <p className="text-slate-500 text-xs mt-3 text-center">총 {totalReviews}개 · 평균 ★{avgRating.toFixed(1)}</p>
                  </>
                )}
              </div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                ...
                {totalFeedback === 0 ? (
                  <p className="text-slate-500 text-sm text-center py-8">피드백 없음</p>
                ) : (
                    ...
                    {Object.entries(feedbackTypes)
                      ...
                      .map(([type, count]: any) => {
                        ...
                        return (
                          <div key={type}>
                            <div className="flex items-center justify-between mb-1">
                              <span className={`text-xs ${FEEDBACK_TEXT[type] || 'text-slate-400'}`}>{FEEDBACK_LABELS[type] || type}</span>
                              <span className="text-white text-xs font-semibold">{count}건 ({pct}%)</span>
                            </div>
                            <div className="w-full bg-slate-800 rounded-full h-2">
                              <div className={`${FEEDBACK_COLORS[type] || 'bg-slate-500'} h-2 rounded-full transition-all`}
                                style={{ width: `${pct}%` }} />
                            </div>
                          </div>
                        )
                      })}
                    <p className="text-slate-600 text-xs text-center pt-1">총 {totalFeedback}건의 피드백</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {bugSeverityDist.length > 0 && (
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                <h3 className="text-white font-semibold mb-4 flex items-center gap-2">
                  <Bug className="w-4 h-4 text-red-400" /> 버그 심각도 분포
                </h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {bugSeverityDist.map((b: any) => (
                    <div key={b.severity} className="text-center bg-slate-800/50 rounded-xl p-4">
                      <div className={`w-3 h-3 rounded-full ${SEVERITY_COLORS[b.severity] || 'bg-slate-500'} mx-auto mb-2`} />
                      <p className="text-white font-bold text-lg">{b.count}</p>
                      <p className="text-slate-400 text-xs">{SEVERITY_LABELS[b.severity] || b.severity}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {topHelpfulReviews.length > 0 && (
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                <h3 className="text-white font-semibold mb-4 flex items-center gap-2">
                  <ThumbsUp className="w-4 h-4 text-green-400" /> 도움됨 TOP 리뷰
                </h3>
                <div className="space-y-3">
                  {topHelpfulReviews.map((r: any) => (
                    <div key={r._id} className="flex items-start gap-3 p-3 bg-slate-800/50 rounded-lg">
                      <div className="w-7 h-7 bg-slate-700 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0">
                        {(r.userId?.username || '?')[0].toUpperCase()}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                          <span className="text-white text-sm font-medium">{r.userId?.username}</span>
                          <span className="text-yellow-400 text-xs">{'★'.repeat(r.rating)}</span>
                          <span className={`text-xs px-1.5 rounded ${FEEDBACK_COLORS[r.feedbackType] || 'bg-slate-600'} bg-opacity-20 ${FEEDBACK_TEXT[r.feedbackType] || 'text-slate-400'}`}>
                            {FEEDBACK_LABELS[r.feedbackType] || r.feedbackType}
                          </span>
                        </div>
                        <p className="text-slate-300 text-sm font-medium truncate">{r.title}</p>
                        <p className="text-slate-500 text-xs">{r.content?.slice(0, 80)}{r.content?.length > 80 ? '...' : ''}</p>
                      </div>
        ...
        {tab === 'reviews' && (
          <div className="space-y-4">
            {/* 필터 */}
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-slate-400 text-sm">유형:</span>
              ...
              <span className="text-slate-600 text-xs ml-auto">최근 10건 표시</span>
            </div>

            {filteredReviews.length === 0 ? (
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 text-center">
                <MessageSquare className="w-10 h-10 text-slate-600 mx-auto mb-3" />
                <p className="text-slate-500">해당 유형의 리뷰가 없습니다</p>
              </div>
            ) : (
              <div className="space-y-3">
                {filteredReviews.map((r: any) => (
                  <div key={r._id} className={`bg-slate-900 border rounded-xl p-4 ${r.isBlocked ? 'border-red-800/40 bg-red-950/10' : 'border-slate-800'}`}>
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <span className="text-white text-sm font-semibold">{r.userId?.username}</span>
                          <span className="text-yellow-400 text-xs">{'★'.repeat(r.rating)}</span>
                          <span className={`text-xs px-1.5 py-0.5 rounded ${FEEDBACK_COLORS[r.feedbackType] || 'bg-slate-600'}/20 ${FEEDBACK_TEXT[r.feedbackType] || 'text-slate-400'} border border-current/20`}>
                            {FEEDBACK_LABELS[r.feedbackType] || r.feedbackType}
                          </span>
                          {r.isVerifiedTester && (
                            <span className="bg-green-600/20 text-green-300 text-xs px-1.5 rounded border border-green-500/30 flex items-center gap-0.5">
                              <Shield className="w-2.5 h-2.5" /> 인증
                            </span>
                          )}
                          {r.isBlocked && (
                            <span className="bg-red-600/20 text-red-300 text-xs px-1.5 rounded border border-red-500/30">차단됨</span>
                          )}
                          {r.bugSeverity && (
                            <span className="text-xs text-orange-400">[{SEVERITY_LABELS[r.bugSeverity]}]</span>
                          )}
                        </div>
                        <p className="text-slate-200 text-sm font-medium mb-1">{r.title}</p>
                        <p className="text-slate-400 text-xs leading-relaxed">{r.content}</p>
                        <div className="flex items-center gap-3 mt-2">
                          <span className="text-slate-600 text-xs">{new Date(r.createdAt).toLocaleDateString('ko-KR')}</span>
                          {r.helpfulCount > 0 && (
        ...
        {tab === 'feedback' && (
          <div className="space-y-4">
              ...
              {Object.entries(feedbackTypes)
                ...
                .map(([type, count]: any) => {
                  ...
                  const avgR = typeReviews.length > 0
                    ...
                    : '-'
                  return (
                    <div key={type} className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                      <div className="flex items-center gap-2 mb-3">
                        <div className={`w-3 h-3 rounded-full ${FEEDBACK_COLORS[type] || 'bg-slate-500'}`} />
                        <span className={`font-semibold text-sm ${FEEDBACK_TEXT[type] || 'text-slate-300'}`}>{FEEDBACK_LABELS[type] || type}</span>
                        <span className="ml-auto text-white font-bold">{count}건</span>
                      </div>
                      <div className="w-full bg-slate-800 rounded-full h-2 mb-4">
                        <div className={`${FEEDBACK_COLORS[type] || 'bg-slate-500'} h-2 rounded-full`} style={{ width: `${pct}%` }} />
                      </div>
                      <div className="grid grid-cols-2 gap-2 text-center">
                        <div className="bg-slate-800/50 rounded-lg p-2">
                          <p className="text-white font-bold">{pct}%</p>
                          <p className="text-slate-500 text-xs">전체 비율</p>
                        </div>
                        <div className="bg-slate-800/50 rounded-lg p-2">
                          <p className="text-white font-bold">★ {avgR}</p>
            ...
            {bugSeverityDist.length > 0 && (
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                <h3 className="text-white font-semibold mb-4 flex items-center gap-2">
                  <Bug className="w-4 h-4 text-red-400" /> 버그 심각도 상세
                  <span className="ml-auto text-slate-500 text-xs">총 {bugSeverityDist.reduce((s: number, b: any) => s + b.count, 0)}건</span>
                </h3>
                <BarChart
                    ...
                    .map((r: any) => (
                      <div key={r._id} className="flex items-start gap-2 p-3 bg-blue-950/20 border border-blue-800/30 rounded-lg">
                        ...
                        <div>
                          ...
                          <p className="text-slate-400 text-xs mt-0.5">{r.content?.slice(0, 120)}{r.content?.length > 120 ? '...' : ''}</p>
                          <p className="text-slate-600 text-xs mt-1">by {r.userId?.username} · {new Date(r.createdAt).toLocaleDateString('ko-KR')}</p>
                        </div>
                      </div>
                    ))}
                </div>
            ...
            {totalFeedback === 0 && (
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-16 text-center">
                ...
                <p className="text-slate-500">피드백 데이터가 없습니다</p>
              </div>
            )}
          </div>
        )}
