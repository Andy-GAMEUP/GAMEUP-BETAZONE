...
export default function PlayerGameDetailPage() {
  ...
  const thumbUrl = game?.thumbnail ? `/uploads/${(game.thumbnail as string).replace('uploads/', '')}` : GENRE_IMG[(game?.genre as string) || ''] || GENRE_IMG.default
  ...
  return (
    <div className="min-h-screen bg-slate-950">
      ...
      <div className="relative h-64 md:h-80 overflow-hidden">
        <img src={thumbUrl} alt={game.title as string} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = GENRE_IMG.default }} />
        <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/60 to-transparent" />
        <div className="absolute bottom-0 left-0 right-0 p-6">
          <div className="max-w-5xl mx-auto flex items-end justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                <span className="bg-cyan-600/30 text-cyan-300 border border-cyan-500/40 text-xs px-2 py-0.5 rounded">{game.genre as string || '기타'}</span>
                {game.approvalStatus === 'approved' && <span className="bg-green-600/30 text-green-300 border border-green-500/40 text-xs px-2 py-0.5 rounded">승인됨</span>}
                {game.status === 'archived' && <span className="bg-slate-600/30 text-slate-400 border border-slate-500/40 text-xs px-2 py-0.5 rounded">베타종료</span>}
              </div>
            ...
            <div className="space-y-4">
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                ...
                <dl className="space-y-3 text-sm">
                  <div className="flex justify-between"><dt className="text-slate-400">장르</dt><dd className="text-white">{game.genre as string || '-'}</dd></div>
                  <div className="flex justify-between"><dt className="text-slate-400">수익화</dt><dd className="text-white capitalize">{(game.monetization as string) || 'free'}</dd></div>
                  <div className="flex justify-between"><dt className="text-slate-400">플레이</dt><dd className="text-cyan-400 font-bold">{(game.playCount as number || 0).toLocaleString()}회</dd></div>
                  <div className="flex justify-between"><dt className="text-slate-400">리뷰</dt><dd className="text-purple-400 font-bold">{reviewTotal}개</dd></div>
                  <div className="flex justify-between"><dt className="text-slate-400">평균 별점</dt><dd className="text-yellow-400 font-bold">{avgRating.toFixed(1)} / 5.0</dd></div>
                </dl>
              </div>
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          ...
          <div className="space-y-4">
            {!isAuthenticated ? (
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-10 text-center">
                ...
                <Link to="/login" className="bg-cyan-600 hover:bg-cyan-700 text-white px-6 py-2 rounded-lg font-medium transition-colors">로그인하기</Link>
              </div>
            ) : game.status === 'archived' ? (
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-10 text-center">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              <div className="flex gap-1">
                {[['', '전체'], ['general', '일반'], ['praise', '칭찬'], ['suggestion', '제안'], ['bug', '버그']].map(([val, label]) => (
                  <button key={val} onClick={() => { setReviewFilter(val); setReviewPage(1) }} className={`px-3 py-1 rounded text-xs font-medium transition-colors ${reviewFilter === val ? 'bg-cyan-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white border border-slate-700'}`}>{label}</button>
                ))}
              </div>
              <div className="flex gap-1 ml-auto">
                {[['recent', '최신순'], ['helpful', '도움순']].map(([val, label]) => (
                  <button key={val} onClick={() => setReviewSort(val as 'recent' | 'helpful')} className={`px-3 py-1 rounded text-xs transition-colors ${reviewSort === val ? 'text-cyan-400' : 'text-slate-500 hover:text-slate-300'}`}>{label}</button>
                ))}
              </div>
            </div>
            ...
            {Math.ceil(reviewTotal / 8) > 1 && (
              <div className="flex gap-2 justify-center">
                {Array.from({ length: Math.ceil(reviewTotal / 8) }, (_, i) => i + 1).map((p) => (
                  <button key={p} onClick={() => setReviewPage(p)} className={`w-8 h-8 rounded text-sm ${reviewPage === p ? 'bg-cyan-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}>{p}</button>
                ))}
              </div>
            )}
          </div>
